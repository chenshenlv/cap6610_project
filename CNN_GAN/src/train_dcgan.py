...
mini_train_data, mnist_restset = torch.utils.data.random_split(train_data, [int(0.9 * len(train_data)), int(0.1 * len(train_data))])
...
def train_discriminator(optimizer, data_real, data_fake):
    ...
    output_real = discriminator(data_real).squeeze()
    loss_real = criterion(output_real, real_label)
    # get the outputs by doing fake data forward pass
    output_fake = discriminator(data_fake).squeeze()
